...
<!DOCTYPE html>
    ...
    <h1>Ejemplos Webpay - Transaccion Normal</h1>
        ...
        if (action == null) {      
            ...
            try{
                ...
                amount = (Integer) session.getAttribute("totalWebpay"); 
                //amount = (Integer) 2000;
 ...
 %>  
            ...
            <div style="background-color:lightgrey;">
                    ...
                    <%request.getSession().setAttribute("url", result.getUrl());%>
                    <%if(result.getToken()!=null){  request.getRequestDispatcher("confirmaPago.jsp").forward(request, response);  %>
            <!--<p><samp>Sesion iniciada con exito en Webpay</samp></p>
            <br><form name="confirmaPago" action='<%=result.getUrl()%>' method="post"><input type="hidden" name="token_ws" value='<%=result.getToken()%>'><input type="button" onclick="confirmacion()" value="Ejecutar Pago con Webpay"></form>
            <br>-->                        
                    <%}else{                                    %>                    
      ...
      <%       
            ...
            <p><samp>Pago ACEPTADO por webpay (se deben guardar datos para mostrar voucher)</samp></p>
                    <% }
                    request.getSession().setAttribute("urlRedirecciona", result.getUrlRedirection());
                    request.getRequestDispatcher("verificandoPago.jsp").forward(request, response);
                    %>            
          ...
          <%               
           ...
           <h2>Step: End</h2>
            ...
            <% if (request.getParameter("token_ws") != null) { 
                    request.getRequestDispatcher("finalizarPago.jsp").forward(request, response);
            %>
